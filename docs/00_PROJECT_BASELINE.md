...
-   손익·경비분석용 경비코드는 상품분류와 분리된 계층형 Master로 관리하고,
    실제 거래가 참조한 코드는 물리삭제하지 않는다.
...
-   가격·단가·LOT 개별원가는 소수점 없이 원 단위 정수로 저장·표시하며,
    계산 결과의 소수점 이하는 올림한다.
-   중량은 KG 기준 소수점 둘째 자리까지 저장·표시한다. 예: `256.37kg`
-   BOX 수량은 정수만 저장·표시한다.
-   환율은 소수점 둘째 자리까지 저장·표시한다.
-   계산과 DB 저장에는 부동소수점 오차를 피하도록 `Decimal/Numeric`을
    사용하고, 화면 표시만으로 원본 정밀도를 판단하지 않는다.
...
편의기능보다 데이터 무결성·오류방지·안정성·신속한 반응을 우선한다.
정렬·자동 열 너비·자동완성 같은 UI 기능은 PK/FK 관계, 중복방지, 저장대상
식별, 입력순서 및 기존 정상 기능에 영향이 없음을 확인한 경우에만 적용한다.
고유 ID 연결을 보장할 수 없거나 오류 가능성·성능저하가 커지는 경우에는
편리하더라도 해당 기능을 적용하지 않는다.
