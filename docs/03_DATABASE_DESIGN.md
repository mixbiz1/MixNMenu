...
-   `tb_expense_code`: `expense_id` PK, `expense_code` UNIQUE,
    `parent_expense_id` Self FK, 최대 4단계, 손익구분, `node_type`,
    `formula_code`, `system_yn`

경비코드는 표준 손익구조 아래 `GROUP`, 거래선택용 `INPUT`, 보고서 계산용
`CALCULATED`를 구분한다. 하위코드는 상위코드의 손익구분을 상속한다.
상품분류나 일반 공통코드와 PK/FK를 공유하지 않는다. 참조가 없는 사용자
코드는 삭제하고, 사용된 코드는 삭제 대신 사용중지한다. `INPUT`도 필요하면
하위항목을 가질 수 있으며 거래 Detail에서는 하위항목이 없는 `INPUT`만
선택하여 중복집계를 방지한다.
...
상품매입 화면의 `저장`은 임시저장이 아니라 업무 완료 명령이다. 서버는
`tb_purchase/tb_purchase_item` 저장, 상태 `CONFIRMED`, 창고별 입고 Header,
행별 LOT·입고 Detail, `PURCHASE_PAYABLE`을 하나의 DB Transaction에서 처리한다.
중간 단계가 실패하면 전체를 Rollback하여 매입과 재고가 어긋나지 않는다.

확정 전표 수정은 원문만 덮어쓰지 않는다. 해당 전표가 만든 입고·LOT·미지급
연결을 먼저 회수한 다음 변경된 원문과 파생원장을 같은 Transaction에서 다시
생성한다. 출고 등 후속자료가 LOT나 입고를 참조하면 FK가 회수를 막으며 수정과
취소 모두 거부한다. `DRAFT`와 기존 confirm API는 호환용으로 유지하지만 현재
