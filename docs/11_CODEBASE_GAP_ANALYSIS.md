...
## 2026-09-18 사용자·권한 상태

-   사용자등록과 기존 로그인·비밀번호변경을 연결하였다.
-   비밀번호 응답 제외와 PBKDF2-SHA256 저장, 기존 평문 계정의 로그인 시
    Hash 자동전환을 구현하였다.
-   남은 Gap은 사용자별 업무회사 접근, 역할/메뉴권한, API 인증세션과
    공통 권한검사, 거래별 작성자·수정자 Audit이다.
