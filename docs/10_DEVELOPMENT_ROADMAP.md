...
2026-09-18 사용자등록(조회·신규·수정·초기/재설정 비밀번호·사용중지/재개)
Vertical Slice를 구현하였다. 다음 단계는 사용자별 업무회사 접근과
프로그램 메뉴권한이다. 거래 입력 화면보다 먼저 공통 권한검사를 완성하여
이후 모든 화면에 같은 방식을 적용한다.
...
1.  **사용자·권한 마감**
    - 사용자별 업무회사 접근
    - 역할/메뉴권한과 API 공통 권한검사
    - 거래 저장 시 작성자·수정자 Audit 기준
