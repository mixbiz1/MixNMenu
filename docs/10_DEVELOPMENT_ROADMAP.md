...
3.  **일반 매입 → 입고 → LOT → 재고조회**
    - 2026-09-18 일반 매입 Header/Detail 작성·조회·수정·확정·취소 완료
    - 사용자 검증 반영: 상품매입/경비매입 분리, 상품매입 저장 즉시 국내입고·LOT·미지급 원거래 동시생성
    - 레거시 입력흐름 반영: 날짜별 전표요약과 입력상세를 한 화면에 배치, 거래처·상품 팝업검색, Enter 이동
    - 확정 전표 수정 시 매입이 만든 입고·LOT·미지급을 원자적으로 재구성하며 후속 연결자료가 있으면 차단
    - 다음 단계: 상품매입 재고조회 검증, 후속 출고 연결 및 부분입고가 필요한 계약매입 배분구조
    - 경비매입은 상품매입과 별도 Vertical Slice로 구현
