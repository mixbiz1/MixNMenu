...
# 프로젝트 Root 경로를 sys.path에 추가하여 어디서든 모듈을 찾을 수 있도록 설정
...
# company_reg 모듈 예외 없는 안전한 Import
try:
    from views.company_reg import CompanyRegWidget as CompanyRegView
except ImportError:
    try:
        from company_reg import CompanyRegWidget as CompanyRegView
    except ImportError:
        CompanyRegView = None
...
def load_main_window(user_name=""):
    ...
    try:
        ...
        print(f"[Notice] 메뉴 액션 연결 예외: {e}")

    # 프로그램 시작 시 단축메뉴(대시보드) 자동 열기
...
def open_dashboard(main_window):
    ...
    try:
        ...
        dashboard_widget = MainDashboard(main_window=main_window)
...
def open_company_reg(main_window):
    ...
    if CompanyRegView:
        try:
            ...
            sub_company.setWindowTitle("1.업체등록")
