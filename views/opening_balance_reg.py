...
class OpeningBalanceRegWindow(QWidget):
    ...
    def _build_ui(self):
        ...
        form_box = QGroupBox("최초잔액 입력"); form = QFormLayout(form_box)
        ...
        self.account = QComboBox(); self.account.setMinimumWidth(330)
        ...
        buttons = QHBoxLayout(); self.btn_new = QPushButton("신규 [F2]"); self.btn_save = QPushButton("저장 [F4]"); self.btn_search = QPushButton("조회 [F7]"); self.btn_close = QPushButton("닫기")
        for button in (self.btn_new,self.btn_save,self.btn_search,self.btn_close): buttons.addWidget(button)
        ...
        self.table.setAlternatingRowColors(True); ll.addWidget(self.table); body.addWidget(list_box, 1)

        self.balance_type.currentIndexChanged.connect(self._filter_accounts); self.btn_new.clicked.connect(self.clear_form)
        self.btn_save.clicked.connect(self.save_data); self.btn_search.clicked.connect(self.load_data); self.btn_close.clicked.connect(self.close_window)
        self.btn_new.setShortcut("F2"); self.btn_save.setShortcut("F4"); self.btn_search.setShortcut("F7")
    ...
    def clear_form(self):
        self.base_date.setDate(QDate.currentDate()); self.amount.setValue(0); self.memo.clear(); self.account.setCurrentIndex(0); self.account.setFocus()
    ...
    def save_data(self):
        ...
        if QMessageBox.question(self, "최초잔액 저장", f"{kind} {rounded:,}원을 원거래로 생성하시겠습니까?") != QMessageBox.Yes: return
        ...
        try:
            response = httpx.post(f"{API_BASE_URL}/companies/{app_context.company_code}/opening-balances", json=payload, timeout=15); response.raise_for_status()
            QMessageBox.information(self, "저장 완료", f"원거래 {response.json()['transaction_no']}가 생성되었습니다."); self.clear_form(); self.load_data()
